...
test.describe("标签拖拽功能", () => {
  ...
  test.describe("图像标签拖拽", () => {
    ...
    test("标签拖拽到图像列表视图", async ({ electronTest, page }) => {
      await electronTest.logTestStart();

      const factory = electronTest.getApiFactory();
      const imageFactory = factory.createImageFactory();

      // 创建独立测试图像和标签，避免受前面测试数据影响
      const [testImage] = await imageFactory.createBatch(1, "drag_list");
      const { tagName } = await imageFactory.createTagInGroup("drag_list_group", "drag_list_tag", true);
      await electronTest.refreshData();

      const testImageId = String(testImage.id);
      expect(testImageId).toBeTruthy();
      ...
      await page.waitForFunction(
        (name: string) => {
          return document.querySelector(
            `#imageTagFilterList .tag-filter-item[data-tag="${name}"]`,
          ) !== null;
        },
        tagName,
        { timeout: 1000 },
    ...
    test("标签拖拽到图像紧凑视图", async ({ electronTest, page }) => {
      await electronTest.logTestStart();
      ...
      // 创建独立测试图像和标签，避免受前面测试数据影响
      const [testImage] = await imageFactory.createBatch(1, "drag_compact");
      const { tagName } = await imageFactory.createTagInGroup("drag_compact_group", "drag_compact_tag", true);
      await electronTest.refreshData();
