...
test("图像详情界面 - 点击下拉建议项添加标签", async ({
  ...
  await expect(
    page.locator(
      `#${Constants.Ids.IMAGE_DETAIL_TAGS_CONTAINER} .tag-editable[data-tag="${existingTagName}"]`,
    ),
  ).toBeVisible({ timeout: 1000 });
...
test("图像详情界面 - 下拉框激活时回车添加当前输入（无选中项）", async ({
  ...
  await enterImageTagManager(page);
  const existingTag = electronTest.generateE2ePrefixName("existing_trigger");
  await createImageTagInManager(page, existingTag);
...
test("提示词详情界面 - 点击下拉建议项添加标签", async ({
  ...
  await expect(
    page.locator(
      `#${Constants.Ids.PROMPT_DETAIL_TAGS_CONTAINER} .tag-editable[data-tag="${existingTagName}"]`,
    ),
  ).toBeVisible({ timeout: 1000 });
...
test("提示词详情界面 - 下拉框激活时回车添加当前输入（无选中项）", async ({
  ...
  await enterPromptTagManager(page);
  const existingTag = electronTest.generateE2ePrefixName("existing_trigger");
  await createPromptTagInManager(page, existingTag);
