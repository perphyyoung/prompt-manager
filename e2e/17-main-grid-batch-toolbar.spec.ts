...
test.describe("批量工具栏 - 主界面功能测试", () => {
  ...
  // ==================== 图像主界面 - 视图模式切换保留选择状态 ====================
  test("图像主界面-视图模式切换应该保留选择状态", async ({
    electronTest,
    page,
  }) => {
    await electronTest.logTestStart();
    ...
    const cardId1 = await card1.getAttribute("data-id");
    const cardId2 = await card2.getAttribute("data-id");
    await card1.click();
    await card2.click();
    await page.keyboard.up("Control");
    ...
    await page.waitForFunction(
      (ids: readonly (string | null)[]) => {
        const selectedItems = document.querySelectorAll(
          ".list-item--image.is-selected, .image-list-item.is-selected",
        );
  ...
  // ==================== 提示词主界面 - 视图模式切换保留选择状态 ====================
  test("提示词主界面-视图模式切换应该保留选择状态", async ({
    electronTest,
    page,
  }) => {
    await electronTest.logTestStart();
    ...
    await page.waitForFunction(
      (ids: readonly (string | null)[]) => {
        const selectedItems = document.querySelectorAll(
          ".list-item--prompt.is-selected, .prompt-list-item.is-selected",
        );
        const selectedIds = Array.from(selectedItems).map((item) =>
          item.getAttribute("data-id"),
        );
  ...
  // ==================== 图像主界面 - 紧凑视图切换保留选择状态 ====================
  test("图像主界面-紧凑视图切换应该保留选择状态", async ({
    electronTest,
    page,
  }) => {
    await electronTest.logTestStart();
  ...
  // ==================== 提示词主界面 - 紧凑视图切换保留选择状态 ====================
  test("提示词主界面-紧凑视图切换应该保留选择状态", async ({
    electronTest,
    page,
  }) => {
    await electronTest.logTestStart();
