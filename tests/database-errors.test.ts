...
describe("DatabaseError", () => {
  it("should create DatabaseError with all properties", () => {
    ...
    const error = new DatabaseError("Test error", DatabaseErrorCode.SQL_ERROR, originalError, {
  ...
  it("should filter sensitive data in toJSON", () => {
    const error = new DatabaseError("Test error", DatabaseErrorCode.SQL_ERROR, undefined, {
...
describe("ConstraintViolationError", () => {
  it("should create error with constraint type", () => {
    ...
    const error = new ConstraintViolationError("唯一约束违反", "UNIQUE", originalError);
...
describe("isConstraintError", () => {
  it("should return true for CONSTRAINT_VIOLATION error code", () => {
    const error = new DatabaseError("Constraint failed", DatabaseErrorCode.CONSTRAINT_VIOLATION);
    expect(isConstraintError(error)).toBe(true);
  });

  it("should return true for ConstraintViolationError type", () => {
    const error = new ConstraintViolationError("唯一约束违反", "UNIQUE", new Error("Original"));
    expect(isConstraintError(error)).toBe(true);
  });
