  ...
  describe('handleError', () => {
    ...
    it('customToastType 正确传递给 showToast', () => {
      ...
      ErrorHandler.handleError(
        { module: 'TestModule', operation: 'test op' },
        error,
        { app: customApp, toastType: 'warning' }
      );
    ...
    it('userMessage 覆盖默认消息格式', () => {
      ...
      ErrorHandler.handleError(
        { module: 'TestModule', operation: 'test op' },
        error,
        { app: customApp, userMessage: '自定义错误消息' }
      );
    ...
    it('window.electronAPI 存在但 logError 不存在时不调用', () => {
      const originalElectronAPI = (window as unknown as { electronAPI?: { logError?: unknown } }).electronAPI;
  ...
  describe('handleAsyncError', () => {
    ...
    it('handleAsyncError 不支持 app 参数（由 handleError 传递）', async () => {
      ...
      const result = await ErrorHandler.handleAsyncError(
        { module: 'TestModule', operation: 'test op' },
        operation,
        { app: customApp, defaultValue: 'fallback' } as Parameters<typeof ErrorHandler.handleAsyncError>[2]
      );
