...
export class TagUI {
  ...
  renderTagGroupCards(
    ...
    if (ungroupedTags.length > 0) {
      html += this.generateUngroupedTagCard(ungroupedTags, tagCounts, globalIndex);
    ...
    sortedGroups.forEach((group) => {
      ...
      html += this.generateTagGroupCard(group, tags, tagCounts, isFirst, globalIndex);
  ...
  generateTagItemHtml(
    ...
    const editBtn = `<button class="tag-edit-btn" data-tag="${escapedTag}" title="编辑">
    ...
    const deleteBtn = `<button class="tag-delete-btn" data-tag="${escapedTag}" title="删除">
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      return `
        <div class="tag-group-card ungrouped-card" data-group-id="" data-drop-target="true">
          <div class="tag-group-card-header">
            <div class="tag-group-card-header-left"></div>
            <div class="tag-group-card-header-center">
              <span class="tag-group-card-name">${groupName}</span>
            </div>
            <div class="tag-group-card-header-right"></div>
          </div>
