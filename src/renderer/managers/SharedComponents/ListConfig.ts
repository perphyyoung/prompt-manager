...
export type ListViewTypeValue = typeof ListViewType[keyof typeof ListViewType];
...
export interface LayoutSection {
  ...
  render?: (item: Record<string, unknown>) => string | null;
}

/**
 * 共用标准布局
 */
const STANDARD_LAYOUT: LayoutSection[] = [
  { component: 'checkbox' },
 ...
 * 共用紧凑布局
 */
const COMPACT_LAYOUT: LayoutSection[] = [
  { component: 'checkbox' },
  { component: 'thumbnail', placeholder: true },
  {
    component: 'text-content',
...
export interface ListFields {
  id?: string | ((item: Record<string, unknown>) => string | number);
  title?: string | ((item: Record<string, unknown>) => string);
  content?: string | ((item: Record<string, unknown>) => string);
  tags?: string;
  isFavorite?: string;
...
export interface ListLayout {
  standard: LayoutSection[];
  compact: LayoutSection[];
}

export interface ListConfigOptions {
  type: ListViewTypeValue;
  dataType: string;
  itemType?: string;
...
export class ListConfig {
  ...
  constructor(options: ListConfigOptions) {
   ...
   * modifier: 修饰符（可选）
   */
  getClassName(element?: string, modifier?: string): string {
   ...
   * 获取字段值
   */
  getValue(item: Record<string, unknown>, fieldName: keyof ListFields): unknown {
  ...
  getTitle(item: Record<string, unknown>): string {
    const title = this.getValue(item, 'title');
    return String(title || '无标题');
  }

  /**
   * 获取内容
   */
  getContent(item: Record<string, unknown>): string {
    const content = this.getValue(item, 'content');
   ...
   * 获取缩略图
   */
  getThumbnail(item: Record<string, unknown>): string | null {
    const thumbnail = this.getValue(item, 'thumbnail');
    return thumbnail as string | null;
  }

  /**
   * 获取标签数组
   */
  getTags(item: Record<string, unknown>): string[] {
    return (item[this.fields.tags as string] as string[]) || [];
  }

  /**
   * 获取复选框 data 属性
   */
  getCheckboxDataAttrs(item: Record<string, unknown>, index: number): string {
   ...
   * 获取布局配置
   */
  getLayout(isCompact: boolean): LayoutSection[] {
 ...
 * 列表按钮配置
 */
const LIST_MAIN_BUTTONS: ButtonConfig[] = [
...
export const PromptListConfig = new ListConfig({
  ...
  fields: {
    ...
    note: 'note',
    thumbnail: (prompt: Record<string, unknown>) => {
      const promptWithImages = prompt as { images?: Array<{ thumbnailPath?: string }> };
      return promptWithImages.images?.[0]?.thumbnailPath || null;
    }
  },
...
export const ImageListConfig = new ListConfig({
  ...
  fields: {
    id: 'id',
    title: (img: Record<string, unknown>) => (img as { fileName?: string }).fileName || '无标题',
    content: (img: Record<string, unknown>) => {
      const imgWithRefs = img as { promptRefs?: Array<{ promptContent?: string }> };
      return imgWithRefs.promptRefs?.[0]?.promptContent || '';
    },
    tags: 'tags',
    isFavorite: 'isFavorite',
