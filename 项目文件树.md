...
```bash
main/
├── index.ts                     # 应用主进程入口 (TypeScript)，管理窗口生命周期和 IPC 通信
├── database.js                  # 数据库操作层，封装 SQLite 数据存取
├── logger.ts                    # 日志服务 (TypeScript)，统一输出到 pm.log
└── splash-window.ts             # 启动窗口 (TypeScript)
```
...
│   ├── SimpleTagManagerFactory.js   # 简单标签管理器工厂，创建标签管理器实例
│   ├── TagRegistry.ts               # 标签注册表 (TypeScript)，管理标签数据和业务逻辑
│   ├── ITagRegistry.ts              # 标签注册表接口定义 (TypeScript)
...
│   ├── ImportExportManager.ts       # 导入导出管理器，处理数据导入导出 (TypeScript)
│   │
...
├── components/                  # 组件层
│   ├── index.js                 # 组件统一导出
│   ├── BatchTagManager.js       # 批量标签管理组件，支持批量删除标签
│   ├── EditableTagList.js       # 可编辑标签列表组件，支持增删改标签
│   └── ProgressDialog.ts        # 进度对话框组件 (TypeScript)
│
├── services/                    # 服务层
│   ├── index.js                 # 服务统一导出
│   ├── DataClearApi.js          # 数据清理 API 抽象接口
│   ├── DataClearIpcService.js   # 数据清理 IPC 服务
│   ├── DialogService.js         # 对话框服务，管理确认对话框
│   ├── ElectronDataClearApi.js  # Electron 数据清理 API 实现
│   ├── ElectronTagApi.js        # Electron 标签 API 实现
│   ├── ImageUploadService.js    # 图像上传服务，核心上传逻辑
│   ├── MockDataClearApi.js      # Mock 数据清理 API 实现
│   ├── MockTagApi.js            # Mock 标签 API 实现
│   ├── SafeRatingService.js     # 安全评级服务，处理内容安全过滤
│   ├── TagApi.js                # 标签 API 抽象接口
│   ├── TagAutocomplete.js       # 标签自动完成服务，提供标签输入自动完成功能
│   ├── TagIpcService.js         # 标签 IPC 服务
...
```bash
preload/
└── index.ts                     # Preload 脚本入口 (TypeScript)，定义 IPC 通道
```
...
```bash
types/
└── electron-api.d.ts            # Electron API 类型定义 (TypeScript)
```
...
├── FileUtils.ts                 # 文件工具类 (TypeScript)，提供复制目录等功能
...
| 目录 | 文件数 | 说明 |
|------|--------|------|
| 主进程 | 4 | index.ts, database.js, logger.ts, splash-window.ts |
| preload | 1 | index.ts |
| types | 1 | electron-api.d.ts |
| renderer/config | 2 | 配置目录（含 index.js） |
| renderer/managers | 27 | 业务管理器（含 3 个 TS 文件） |
| renderer/managers/SharedComponents | 7 | 共享组件（含 index.js） |
| renderer/renderer_utils | 5 | 渲染进程工具类（含 index.js） |
| renderer/components | 4 | UI 组件（含 index.js，1 个 TS 文件） |
| renderer/services | 13 | 服务类（含 index.js） |
| src/utils | 11 | 共享工具类（含 index.js，1 个 TS 文件） |
| 根目录 | 2 | constants.js, verify-imports.js |
| **总计** | **77** | **JS: 70, TS: 7** |
